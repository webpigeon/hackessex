...
from hackessex.consumers import connect_blog, disconnect_blog, save_post

##
# JWR - modified from liveblog example
##
...
channel_routing = [
        ...
        route("websocket.connect", connect_blog, path=r'^/faqs/(?P<room_id>[^/]+)/stream/$'),

        # Called when the client closes the socket
        route("websocket.disconnect", disconnect_blog, path=r'^/faqs/(?P<room_id>[^/]+)/stream/$'),

        # Called when the client sends message on the WebSocket
        route("websocket.receive", save_post, path=r'^/faqs/(?P<room_id>[^/]+)/stream/$'),
